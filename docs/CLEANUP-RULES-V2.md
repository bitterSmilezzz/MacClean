...
① `contract` 满足，或由工具自身 prune 语义自证未引用；
② `ownership` 唯一且 `hostState ≠ installed-running`；
③ `restore ∈ {none, auto-cheap}` 且无状态丢失；
④ 判据是**结构标记**（`.ShipIt.`、`*.log.N`、未被 `opt` 链接引用的 Cellar 版本、Apple 的 3 天阈值、
`.savedState` 后缀），**不能是"某个人不用了"**。
...
T1 15 条；T2 17 条；T3 4 条；迁出空间审计 4 条；拆分 1 条（D19）。
...
3. **档位接进 `Recommendation`**：界面分组从"可清理/使用中/需确认/不建议"改为 T0–T3 + 不列出；
   文案带"缺哪一维"。
4. **T0 默认勾选**（D-1）：改 G2，并加反证自检——"任何非 T0 项被默认勾选即失败"。
