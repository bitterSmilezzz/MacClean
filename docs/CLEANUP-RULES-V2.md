...
- **规则表不外置为 JSON**：外置会让"声明与实现一一对应"这条自检失效（自检要能在编译期穷举 52 条）。改为在 `CleanupRules.Rule` 上加显式维度字段。
...
| C4 Homebrew 缓存 | 可清理 | **T1** 变更 | 按 `brew cleanup` 契约：只碰 >120 天下载，不动已装 formulae 的下载物 |
...
| L3 `/private/tmp`、`/var/tmp` | 需确认 | **T0** 变更 | 判据改为「属主==euid **且** >3 天未访问」（Apple 自己的阈值）；现"当前用户可写"用错判据 |
| L4 TemporaryItems | 可清理 | **T0** 变更 | 同 L3 |
...
| D19 gradle daemon log + wrapper dists | 可清理 | **拆分** 变更 | daemon log → T0；`wrapper/dists` 是被项目按版本引用的发行版 → T2 |
...
**汇总**：T0 共 11 条（C7、L3、L4、L5、L6、D8、D11、D12、D13、D14、A4）；
T1 16 条；T2 17 条；T3 8 条（D2、A3、D23、B1、T2、T3、T4、T5，全部 `contract = userData`）。
...
> **硬顺序约束**：步骤 4（T0 默认勾选）**必须晚于**步骤 5（L3/L4 判据改写）。
> 原因：L3/L4 现在的实现是"`/private/tmp` 里当前用户可写的项"，还没有按
> 「属主==euid **且** >3 天未访问」过滤（§3 的 L3 行）。它们的档位已经登记为 T0，
> 但**实现配不上这个档位**——先开默认勾选，就等于把"刚被某个进程写过的临时文件"
> 也自动勾上。自检里那条"T0 名单逐条点名"锁的是名单，锁不了实现是否兑现。
...
3. **档位接进 `Recommendation`**：界面分组从"可清理/使用中/需确认/不建议"改为 T0–T3 + 不列出；
   文案带"缺哪一维"。
4. **T0 默认勾选**（D-1）：改 G2，并加反证自检——"任何非 T0 项被默认勾选即失败"。**依赖步骤 5**。
5. **L3/L4 判据改写**（属主 + 3 天阈值）+ **C4 对齐 brew 契约** + **D19 拆分**。
6. **FDA 引导**（D-2）：权限缺失产出 `ScanIssue`，界面区分"读不到"与"干净"。
