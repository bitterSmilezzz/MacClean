...
### T 废纸篓（T1）与空间审计（T2–T5）
| 规则 | 现状 | v2 | 依据 |
|---|---|---|---|
| T1 `~/.Trash` | 可清理 | **T1** 变更 | 位置即用户意图，但仍留在清理页、不进"一键处理"（刚扔的东西要能找回） |
| T2 `~/Downloads` 大/旧 | 需确认 | **迁出 ✅ v1.72.12** | D-3：进「空间审计」 |
| T3 大文件 >1GB | 需确认 | **迁出 ✅ v1.72.12** | D-3 |
| T4 模拟器设备 >90 天 | 需确认 | **迁出 ✅ v1.72.12** | 整个模拟器（含其内数据），属审计对象；与 v1.73 的 Android AVD 议题同类 |
| T5 iOS 备份 >180 天 | 需确认 | **迁出 ✅ v1.72.12** | 不可重建的用户数据 |
...
7. **空间审计**（D-3）：✅ 已完成（v1.72.12）。T2/T3/T4/T5 迁出，清理页不再出现"凭大小年龄判"的项。
   实现方式：`Rule.auditOnly` 标记 + 在 `Scanner.scanDetailed` **一处**分流到
   `ScanOutcome.auditItems`（散在 6 个扫描函数里各判各的，漏一处就破功且没人会去查第 7 处），
   新目的地「空间审计」渲染证据、去处与「在访达中显示」，**没有删除按钮也没有勾选框**。
   连带：`.largeFiles` 分类标题改「废纸篓」、`ruleRef` 只算清理页真的会列出的规则（→ "T1"），
   规则本身仍在册（编号连续性由原自检守住）。
   **留给下一轮的尾巴**：废纸篓页顶部仍挂着「工程透视 / 整理下载 / 整理截图」这类
   大文件时代的胶囊与筛选条（`CategoryDetailView` 里 14 处 `category == .largeFiles` 分支）。
   它们本身是空间工具，语义上该跟着审计页走，但那是一次 UI 搬迁，不混进这次数据分流。
