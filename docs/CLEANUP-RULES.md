...
| **G9** | **TCC 权限显式判定（v1.1，v1.72.11 加强）** | 受隐私保护目录读取会返回 `Operation not permitted`。**必须区分「读不到」与「真的空」**，见 §7.2。v1.72.11 起盲区下沉到**子目录级**：测量遍历与 `children(of:)` 撞到的每一次 `EPERM`/`EACCES` 都记账并产出 `ScanIssue`，界面上给出「打开系统设置 / 重新扫描」两个动作 |
...
| T2 | `~/Downloads/*` | 大于 500MB 或超过 180 天未访问 | 用户数据 | **只报告** → 「空间审计」 |
| T3 | 大文件扫描 | `~/Downloads`、`~/Documents`、`~/Desktop`、`~/Movies` 中 >1GB 文件，深度 ≤2 | 用户数据 | **只报告** → 「空间审计」 |
| T4 | `~/Library/Developer/CoreSimulator/Devices/*` | 未使用模拟器（按最后使用时间 >90 天） | 推断未用 | **只报告** → 「空间审计」 |
