...
| G3 | 默认移入废纸篓 | 清理默认 `trash`（可恢复），用户可选「彻底删除」。<br>**可回退的前提是记录还在**（v1.73.2）：撤销快照靠 `CleanUndoSession.recordID` 挂在历史记录上，历史被抹掉 = 快照失去入口 = 用户无从放回。因此**写历史与写快照各只有一个原子入口**（`HistoryStore.append`/`clear`、`UndoManagerStore.record`，内部统一走带事务锁的 `mutate`），两个存储的 `save` 已降为 `private`——**由编译器保证**"不许从别处整片写回"，源码不变量自检只作兜底；`UndoManagerStore.restore` 也不再"读一份数组—搬文件—整片写回"，改成搬完再重新读盘落账，且落账失败会明确报错（否则文件已搬回、快照仍标未还原，那一行既不显示放回也不显示已放回，再点永远报失败）。此前 6 处各自 `load → insert → save`，其中 `AppState` 拿启动时读的那份内存缓存整片写回，会把运行期间其他模块记的记录无声抹掉。<br>**限制**：`NSLock` 只在单进程内有效。launchd 经 `LaunchAgentManager` 拉起的 `--autoclean` 无头进程与 GUI 共用同一份状态文件，跨进程仍是 last-writer-wins（收窄为窄窗口，未消除）；一条记录与其快照也不在同一临界区，中途进程被终止会留下"有记录、无快照"的行，界面按无快照降级、不给放回按钮 |
...
| **G8** | **系统级硬保护（v1.1 · v1.73.8 扫描侧收口）** | `/System`、`/System/Volumes`、`/Library/Updates`、`/private/var/vm`、`/private/var/db`、`/private/var/folders/zz` 一律不列为可清理项。判据见 §7。**v1.73.8**：删除侧（`isSafeToClean` / `governanceVerdict`）早就走 `normalizePath` + `GuardPath.matches`（精确或子层级），扫描侧却仍散着 7 处字符串护栏（Screenshots/Downloads/QuickLook/AppLocalization×2/FontCache sibling/StartupItemInspector/SpaceArchiveService）——G14 曾写"这类弱护栏一律删除"，实际只对 3 张**卡片**各立了单文件 lint。字符串护栏**漏** 4 条非 `/System` 前缀的 SIP 条目（`/private/var/db`、`/private/var/vm`、`/private/var/folders/zz`、`/Library/Updates`）——**实测的 P0 症状**：把 DownloadsOrganizer 换回旧形态跑一次变异，`scan(customDirectory: "/private/var/db")` 返回 **15 个清理候选**（`SystemKey`、`ionodecache.json`、`BootCache.data`、`mtrecorder.enable` 等 SIP 顶层文件被列进面板"下载可清理"），`.skipsSubdirectoryDescendants` 意味着只列顶层就能拿到。删除侧 `ResidueDeletionGate` 兜得住、不会误删；谎报的是"这里 15 项可以清"这件事本身。字符串护栏**过** 拦只有假想的 `/SystemFoo`；`/System/Volumes/Data/Users/...`（firmlink 用户数据）**新旧两版都拦**（`/System/Volumes` 在清单里，本轮未动这条策略）。本轮把 7 处一律换成 `FileSystem.isSystemProtected`（内部就是删除侧那套），并立**全仓 lint G18** 钉死这条模式不再长回来——见 G18 行 |
| **G9** | **TCC 权限显式判定（v1.1，v1.72.11 加强，v1.73.3 加读取截止）** | 受隐私保护目录读取会返回 `Operation not permitted`。**必须区分「读不到」与「真的空」**，见 §7.2。v1.72.11 起盲区下沉到**子目录级**：测量遍历与 `children(of:)` 撞到的每一次 `EPERM`/`EACCES` 都记账并产出 `ScanIssue`，界面上给出「打开系统设置 / 重新扫描」两个动作。v1.73.3 补上第三种形态：授权请求**未决**时 `opendir` 不返回错误，而是停在内核 `__open` 上永不返回——扫描本来就要读的那些门禁位置（T1 废纸篓、T2 下载顶层、T3 四个大文件根、C1/C6/A1 三处候选目录）因此带 **5 秒读取上限**，超时即跳过并记为盲区（`nil` 表示"这轮没看清"，与"读到空"的 `[]` 严格区分）。递归热路径与其余 `children/subdirs` 仍无截止，见 §7.2 ⑤ |
...
| **G14** | **删除只有一个入口（v1.72）** | 任何删除动作都必须经 `ResidueDeletionGate`。主目录**之外**的目标必须先声明所属**治理域**（`GovernanceDomain`：精确根 + 最小层级 + 可选入口名单），由 `FileSystem.governanceVerdict(_:domain:)` 做唯一裁决：软链防跳板 → G8 → G6 → 用户白名单 → 域内且深度足够 → `canUnlink`。<br>**背景**：v1.53–v1.71 的十余个治理模块扫的是 `/Library/Fonts`、`/Library/Printers` 等主目录外位置，而 `isSafeToClean` 的常规允许根只有主目录与临时目录，于是各模块自造 `hasPrefix("/System")` 字符串护栏——**用户白名单与 G6 对它们完全失效**，且软链可绕。现在这些弱护栏一律删除。<br>**永不授权删域根本身**；`/Library/*` 在真机多为 root 只读，网关返回 `needsPrivilege`，UI 必须如实说明"本工具不提权"而不是含糊报"清理失败"。<br>**v1.73.0 补上最后两个例外**：`DevProjectScanner`（开发工程产物）与 `PreferenceResidueInspector`（偏好碎片，由 App 卸载器调用）是 v1.72.0 那轮收敛漏下的。前者自写三条字符串防线后直接 `removeItem`/`trashItem`，删完不写历史与撤销快照，而它单次可释放量最大（真机 8.6 GB）；**后者是 G1–G17 里最脏的一处**——循环里只 `fileExists` 就动手，`isSafeToClean` 一次都没调，于是 G8 系统硬保护、G6 用户数据硬排除与用户自定义白名单对这条路径**完全失效**，`freed` 累加的还是扫描缓存的 `item.size`。现在**治理模块零例外**，且各有源码接线自检：模块内出现裸删除调用、或调用方传 `journal: .none`，自检即红。这条 G14 从此不再只靠人 review 维持。<br>仍不经网关的只剩 `Cleaner`（分类主链路，自带护栏与记账）、`SpaceArchiveService`（校验压缩包完整性后才移走原件）、`LaunchAgentManager`（只删本 App 自己写的 plist）与应用自身状态文件——都不是"扫出候选项再删" |
| **G15** | **子进程必须受控（v1.72）** | 外部命令一律走 `SafeProcess`：① 读端先挂后台排空再等退出（macOS 管道缓冲仅约 64 KB，反序即父子互等死锁）；② 必须有超时（TERM→KILL）；③ 启动失败绝不 `waitUntilExit()`（在未启动的 `Process` 上调用会抛异常崩溃）；④ 命令可用性先 `isAvailable` 判定，**没执行就不算成功**。自检经 `SafeProcess.runner` 注入，断言命令与参数而不真改系统状态。<br>**v1.73.1 补上最后一处例外**：`SpaceArchiveService` 的两处 `ditto`（原位归档、外接盘迁移）当时漏迁，且只在失败分支读管道——归档一个含大量不可读文件的目录即永久卡死，界面"正在归档…"永不复位。本条此前已把 `ditto` 写进"已收敛"名单，属于文档谎报。迁移时**超时须按命令真实耗时显式设定**（`ditto` 取 60 分钟），沿用 10 秒默认值只是把"永远卡住"换成"永远失败"。<br>顺带查出：迁移侧的 `--sequesterRsrc` 只在 PKZip（`-c -k`）下合法，纯复制时 ditto 在解析参数阶段就退出——**"迁移到外接盘"此前从未成功过一次**，而其失败分支无条件 `removeItem(目标卷/<源同名>)`，会永久删掉用户外接盘上已有的同名目录。现在复制形态不带该 flag、落点做重名唯一化、失败只清本次新建的那一份。<br>现在**产品源码全子树**（`Sources/MacClean` 及其下目录，`Selftests/` 除外）里 `Process()` 只出现在 `SafeProcess.swift` 一处，由一条递归遍历源码的自检守住——注意它必须用 `subpathsOfDirectory`：`contentsOfDirectory` 不递归，会把 `Rules/` 整个漏掉 |
...
| **G18** | **扫描侧与删除侧必须读同一份护栏清单（v1.73.8）** | G8 那份 `CleanPaths.systemProtected` 清单**只此一处**被消费：`FileSystem.isSystemProtected`（内部 `normalizePath` + `GuardPath.matches` 的"精确或子层级"）在**扫描根**判定与**删除侧** `isSafeToClean` / `governanceVerdict` 里读同一段代码；**禁止**产品源码的任何一处再用 `hasPrefix("/System")`、`contains("/System")`、`== "/System"`、`starts(with: "/System")` 这类**字符串比较**做 SIP 判据——那是"同一份 G8 清单被两套判据各读一次"，历史上一度让扫描侧漏掉 4 条非 `/System` 前缀的 SIP 条目（`/private/var/db`、`/private/var/vm`、`/private/var/folders/zz`、`/Library/Updates`），实测变异跑一次旧形态就能让 DownloadsOrganizer 把 `/private/var/db` 顶层 15 个 SIP 文件列成清理候选。判据由**全仓 lint**（`Selftest+ScanDiagnostics.swift` 里名 `G18 不变量`）钉住：非注释产品代码里四种字面都不许出现；needle 匹配用**挤过空白**的版本防 swift-format 折行漏检；`mustBeThere` 里 5 个扫描根文件必须各至少出现一次 `FileSystem.isSystemProtected(`，把 guard 整段删掉也判红（防"这里根本不管"的假绿）；活性证据是**每文件 30% 保留比例**——全仓下界对"某个文件被注释剥离器吞到 EOF"完全瞎，单文件比例敏感（v1.73.8 二次复审 P2 实测）|
| **G19** | **路径归一化与判据的单一来源（v1.73.9）** | 产品源码里做**路径判定**（护栏清单比对、字典 key、聚合合并）一律走 `FileSystem.normalizePath`，**禁止** `standardizingPath`——后者会依路径是否存在决定是否解析 `/private` 别名（`FileSystem.swift:1058` 自述历史导致过 7 项清理与护栏测试失败）。同一份护栏清单（G8/G6/白名单）在**扫描/UI/服务**三侧只能有一处判据：`FileSystem.isSystemProtected` / `coreGuardVerdict` / `SpaceArchiveService.isSafeToArchivePath`，**不许**在模型或 View 里再手写一份危险根清单——UI 与服务判据漂移的后果是"按钮开放给服务侧会拒的路径"（G13/G18 一路在消灭的"UI 说能清、护栏说不能"）。判据由**全仓 lint G19**（`Selftest+ScanDiagnostics.swift`）钉住：非注释产品代码里不许出现 `standardizingPath`；`mustBeThere` 正向哨兵要求 `SpaceVisualizerModel.swift` 至少引用一次 `SpaceArchiveService.isSafeToArchivePath(`，撤掉委托也判红。端到端断言用**判别性 SIP 路径**（清单里、旧手写清单里都没有的，如 `/private/var/db`），并按 `MEMORY: feedback-reproduce-reviewer-causality` 跑一次 OLD 变异证明**真的会红**——不用两侧都拒的样本（`/System` 之类）冒"变异性"的名 |
...
## 5. 废纸篓（T1）与空间审计（T2–T5）

| 规则 | 路径模式 | 条件 | 本质 | 去处 |
|------|----------|------|------|------|
| T1 | `~/.Trash/*` | 废纸篓内容 | 用户数据 | 清理页「废纸篓」· 彻底删除（已在垃圾箱） |
| T2 | `~/Downloads/*` | 大于 500MB 或超过 180 天未修改 | 用户数据 | **只报告** → 「空间审计」 |
| T3 | 大文件扫描 | `~/Downloads`、`~/Documents`、`~/Desktop`、`~/Movies` 中 >1GB 文件，深度 ≤2 | 用户数据 | **只报告** → 「空间审计」 |
| T4 | `~/Library/Developer/CoreSimulator/Devices/*` | 未使用模拟器（按最后修改时间 >90 天） | 推断未用 | **只报告** → 「空间审计」 |
| T5 | `~/Library/Application Support/MobileSync/Backup/*` | 超过 180 天未更新的设备备份 | 用户数据 | **只报告** → 「空间审计」 |

> **为什么"闲置"一律按 mtime 而不是 atime（v1.73.3）**：macOS 的 atime 是 lazy 更新的
> （`relatime` 语义，且很多卷干脆挂着 `noatime`），"上次访问时间"既可能落后真实使用几小时到
> 几天，也可能压根不动；更糟的是**测量本身会污染它**——`FileSystem.size(at:)` 遍历目录就会
> 刷新 atime，于是"我看过它"变成"它被用过"。判据只认 mtime：读不到 mtime 时明确**不判闲置**，
> 而不是回退成 `.distantPast`（那等于把"不知道"当成"很久没动"，正是要删用户文件的方向）。
> 因此**凡是我们自己下的"闲置"结论**，文案一律说「未修改 / 未更新」，不写「未访问 / 最后使用」。
> 范围要说清：本文件与 `CleanupRules.swift` 里仍有「Apple 承认 3 天未访问即可清」这类句子，
> 那是在**转述系统自己的策略**（`confstr` / `dirhelper.plist`），不是我们的判据文案，不在此列。
> 已按这条口径改完的是 T2（180 天）与 T4（90 天模拟器）。

> **为什么 T2–T5 不在清理页（v1.72.12，规则 v2 步骤 7 / 决策 D-3）**：
> 这四条的判据是"很大"或"很久没动"，而大小和年龄与"这是垃圾"**没有因果**。
> 把它们放进一个带勾选框和「清理已选项」按钮的页面，等于用界面语言替用户下结论。
> 现在它们在「空间审计」页出现，那里给的是：是什么、多大、多久没动、凭什么列出来、
> 真要处理该走哪条路（模拟器走 Xcode、iOS 备份走 访达/iPhone 镜像的备份管理），
> **不给删除按钮**。分流收在 `Scanner.scanDetailed` 一处，由一条自检整体钉住。
> 分类标题也随之从「大文件与垃圾箱」改成「废纸篓」——列表里只剩废纸篓时还叫前者，
> 就是界面在替工具说谎。
...
>
> **v1.72.11（规则 v2 步骤 6）把这件事下沉到子目录级**，四条实测结论都进了代码注释：
> ① 根目录可读 ≠ 里面可读。本机 `~/Library/Caches` 一切正常，而 `CloudKit`、
>    逐个沙盒容器的 `Data/Library/Caches` 是 `EPERM`——一轮扫描实测 **16 处**这种盲区，
>    此前一处都不会报出来（它们藏在"可读的根"下面）。
> ② **缓存会把盲区固化成"干净"**。`measure(at:)` 命中跨会话增量指纹缓存时直接返回上次的
>    0 字节，而指纹只看目录自身的 `lstat`——被权限挡住的那次遍历结果会永久复用，
>    之后再也不会走进去、也就再也不会报错。所以盲区要**主动问**，不能只等遍历顺手记。
> ③ 主动 `open` 别的 App 的容器会让 macOS 弹出
>    **「MacClean 想访问其他 App 的数据」模态对话框**。人在屏幕前时弹得正好；
>    `DiskMonitor` 每 3 小时的无人值守扫描、以及 `AutoCleanService` 那一轮**必须关掉**
>    （`FileSystem.proactiveBlindSpotProbe`），否则没人点，整轮扫描挂在那里。
> ④ 探测用 `opendir`+`closedir`，不用 `contentsOfDirectory`（后者要把顶层条目全读一遍，
>    而紧随其后就要为同一路径做一次全量遍历）；也不用 `access(R_OK)`——TCC 是在**打开**
>    那一刻拒绝的，`access` 只看 Unix 权限位，对 `CloudKit` 会答"可以"，正好漏掉这一类。
>    实测代价：150 个候选目录 2.1 ms，只有同批体积测算（111.2 ms）的 **2%**。
> ⑤ **v1.73.3：授权请求「未决」时，读取根本不是错误，而是不返回。** 关掉主动探测只挡住了
>    "我们**额外**去开一次"，挡不住扫描**本来就要读**的那些目录：`~/Downloads` 挂着一个再没人应答的
>    TCC 请求时（屏幕上并没有等着点的对话框，它已经成了孤儿），T2 列举下载目录会停在
>    `FileSystem.children → contentsOfDirectory → __open` 上永不返回——`--scan`、`--selftest`、
>    GUI 点「扫描」全部 0% CPU 卡死（采样 2335/2335 同一帧）。
>    现在带截止的读取覆盖这些位置：T1 废纸篓列举、T2 下载顶层列举、T3 四个大文件根的探测、
>    以及 C1/C6/A1 三处"候选目录本来就要 `size(at:)`"的跳过判定（后三处此前因为探测默认值翻成
>    `false` 而**连带丢掉了跳过语义**，本轮补回，且不受那个开关管）。
>    v1.73.4 再把两张归档面板纳进来：`DownloadsOrganizerScanner` / `ScreenshotsOrganizerScanner`
>    原先用无截止的 `FileManager.enumerator` 读桌面/下载/图片，读不到时**返回一个空 summary**，
>    界面于是显示「未发现符合条件的下载文件」——那是把"没读到"报成"这里没有文件"。
>    现在根读取走 `FileSystem.probeDirectory`，它保留**三种**结局：读得到 / 读不到 /
>    **本轮没顾上**（在途额度已满，根本没去 `open`）。前一种进 `unreadableRoots`（配锁形图标与
>    警示），第三种进 `deferredRoots`（中性"请稍后重试"，不提权限）——把第三种也说成权限不足，
>    就是对本机从未发生过的失败编造一条告警，会连带把真该授权的位置的信用一起稀释。
>    **仍未覆盖**：面板列表里每个 `.app`/`.pkg` 条目算体积走的仍是一次无截止的递归枚举，
>    卡死可能只是从根挪到条目上。（v1.73.5 把那份重复实现 `AppLocalizationScanner.directorySize`
>    删掉、统一走 `FileSystem` 的测量入口，于是"走不通的子目录"会记进盲区而不再静默按 0 计；
>    且被权限挡过的**残缺结果不再写进跨会话指纹缓存**——否则那个偏小的数会被当权威值复用最长
>    7 天。但"无截止"这一点没有改变。）
>    三个连带设计：超时返回 `nil` 而"读到空"返回 `[]`，不许混（混了就把"没等到"报成"没东西"）；
>    卡过的目录按 **10 分钟 TTL** 短路且**不按轮清空**（按轮清 = 每轮对每个仍卡死的目录
>    再漏一条收不回来的线程）；在途并发上限 4，令牌耗尽时**跳过但不记盲区**——
>    对没碰过的目录声称"读不到"是凭空造一条权限告警，而谎报比漏报糟。
>    **仍未覆盖**：`children(of:)` 的递归子树与其余约 38 处 `children/subdirs` 依旧无截止，
>    因为给热路径每次读取都跳一趟线程，会把扫描本身变成瓶颈。
...
- **v1.72.12（2026-09-22）**：规则 v2 **步骤 7（决策 D-3）——凭大小/年龄判的项迁出清理页**。
  T2（下载目录大/旧）、T3（>1GB 文件）、T4（>90 天未用的模拟器）、T5（旧 iOS 备份）
  从此**不在删除列表里出现**：它们的判据是"很大"或"很久没动"，与"这是垃圾"没有因果关系，
  而一个带勾选框和「清理已选项」按钮的页面本身就在替工具表态。
  - 新增 `Rule.auditOnly`（默认 false，只这 4 条为 true），分流收在 `Scanner.scanDetailed`
    **一处**：`ScanOutcome` 拆成 `items`（可删）与 `auditItems`（只报告）。
    刻意不散进 6 个扫描函数——漏一处就破功，而没人会想到去查第 7 处。
  - 新增目的地「空间审计」（⌘9 / 侧边栏，与「空间透视」并列）：给"是什么、多大、多久没动、
    凭什么列出来、真要处理走哪条路"（模拟器走 Xcode → Platforms，iOS 备份走 访达/iPhone 镜像
    的备份管理），外加「在访达中显示」；**没有删除按钮，也没有勾选框**。
  - 连带修正：`.largeFiles` 分类标题「大文件与垃圾箱」→「废纸篓」（列表里只剩 T1 时前者就是说谎）；
    `CleanCategory.ruleRef` 改为只统计清理页真的会列出的规则 → 该分类显示 "T1"，
    而 5 条规则仍在册、编号连续性由原自检守住。
  - 留给下一轮的尾巴：废纸篓页顶部仍挂着大文件时代的胶囊与筛选条（`CategoryDetailView` 里
    14 处 `category == .largeFiles` 分支），语义上该跟审计页走，但那是一次 UI 搬迁，
    没有混进这次数据分流。
  - 自检 560 → 564：只报告名单逐条点名（并反证它绝不与 T0 重合）、只报告规则不散落进别的分类、
    largeFiles 真机分流（两桶不重叠、审计桶里不许混可删项）、标题与 ruleRef 同步收窄、
    审计页渲染后既无删除入口也不缺"凭什么列出来"。
- **v1.72.11（2026-09-22）**：规则 v2 **步骤 6（决策 D-2）——权限盲区必须显式报出**。
  「读不到」以前只在**根目录**这一层检测（`probeRoot`），而本机实测的 16 处盲区全都在
  可读的根下面：`~/Library/Caches/CloudKit`、逐个沙盒容器的 `Data/Library/Caches`、
  `~/Library/Application Support` 里被 TCC 拒掉的子目录。它们的表现和"这里没东西"完全一样。
  - 测量遍历与 `children(of:)` 本来就撞见每一次 `EPERM`/`EACCES`，此前一律
    `errorHandler: { _, _ in true }` 丢掉；现在顺手记进 `FileSystem.deniedAccessSnapshot()`
    （父子合并、64 条封顶、**G6/G8 主动硬排除的位置不算盲区**——对照片图库喊"去授权"
    只会把用户训练成忽略这条提示）。
  - 只等遍历报错还不够，见上面 §7.3 的结论 ②：增量指纹缓存会把被拒的 0 字节固化成
    "干净"。因此 C1/C6/A1 在把目录当候选项判定时**主动问一次** `canOpenDirectory`。
  - 措辞按权限口径分开（D-2 原话）：没授权才说"去勾选 MacClean 然后重新扫描"；
    已授权仍读不到就说"属于其他 App 的沙盒或需要管理员，本工具不代你提权"。
    此前 `needsFDA` 的判据是"在不在那份**四条** `tccProtected` 清单里"，
    Safari 容器这类同样要 FDA 的位置不在清单内 → 用户拿到的是一句没有下一步的话。
  - 界面：横幅新增「打开系统设置」按钮（直达 隐私与安全性 → 完全磁盘访问权限），
    概览页的"N 个位置读不到"从装饰文字变成可点动作；
    **空结果页在有盲区时不再写"此分类当前是干净的"**，改说"本次没有可清理项（结果不完整）"。
  - 真机验证时发现并堵掉一个自己引入的挂起风险：主动 `open` 别的 App 容器会让 macOS 弹
    **模态**授权框，而 `DiskMonitor` 每 3 小时无人值守扫一轮、`AutoCleanService` 也会自己扫
    ——没人点就把整轮扫描挂住。新增 `FileSystem.proactiveBlindSpotProbe`（带锁），
    无人值守路径关掉，用户亲手扫描时打开。
  - 自检 551 → 560：盲区记账（含"非权限错误不算盲区"的反证）、父子合并与封顶、
    硬排除位置不喊话、按分类归账、两分支措辞、开关两态、空结果页文案、
    以及一条**性能反证**：探测开销 ÷ 同一路径体积测算 < 0.5（实测 0.02）。
    这条比值一开始测出 5.13 倍，是因为分母被 `measure` 的两级缓存打成了空调用——
    拿缓存命中当基准，任何新增 I/O 都会被判成 regression，已在计时前先失效缓存。
