...
- **重复与相似大文件查找（SHA-256 分块哈希 + 智能衍生词干识别引擎）**：
  ...
  - **智能保留推荐与差异说明**：对完全一致副本推荐保留修改时间最早者，对相似衍生大文件优先推荐保留高清/高容量源文件或主规范文件，并为各行标注推荐/建议原因；
  - **原生分类筛选分段器**：支持在「全部」、「完全一致」与「相似衍生」间平滑切换查看，独立统计各分类浪费体积；
