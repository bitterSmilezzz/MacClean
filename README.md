...
- **用户自定义白名单管理面板（防误扫误删）**：
  - **全链路排除**：支持自定义添加保护路径、文件夹或 App 名称，被保护项不仅在扫描结果中完全隐藏，底层 `FileSystem.isSafeToClean` 与 `Cleaner` 执行器亦层层设防，严格杜绝误删；
  - **原生偏好设置面板**：按下 `⌘,` 即可通过原生分段选择器进入「白名单保护」标签页，支持直接调用 macOS `NSOpenPanel` 原生文件夹/文件选取器，或手动输入路径（支持 `~` 展开）；
  - **右键一键加入保护**：在分类详情页清理项或 App 卸载器关联文件上右键，即可点击「加入白名单排除」，即刻从当前待选列表中移除并持久化保护。
