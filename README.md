...
- **重复文件查找与安全去重（多级哈希引擎）**：
  - **高效哈希比对**：首轮基于文件大小快速聚类，次轮读取头部 8KB 快速校验，终轮采用 `CryptoKit.SHA256` 真实分块流哈希确认，精准比对无误判；
  - **智能副本识别与保留标记**：自动标记最早创建/修改或规范路径的副本为「原文件」，其余副本标记为多余；
  - **一键智能勾选与安全清理**：一键勾选所有重复副本（自动保留原文件），支持批量安全移入废纸篓或彻底清理，实时统计可释放容量；
  - **原生分组视图**：卡片式展示重复分组与各副本路径、修改时间，支持右键「在访达中显示」、「拷贝路径」与「加入白名单排除」。
- **用户自定义白名单管理面板（防误扫误删）**：
