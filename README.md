...
- 覆盖 **531 项自动化自检（100% 绿灯，0 失败，约 20 秒）**：格式化、路径展开、安全护栏、勾选逻辑、Cleaner 双模式、风险徽标、弹窗默认/切换/警告、空态禁用态、历史记录、卸载器交互、启动项治理、APFS 硬链接无损去重、底层快照与休眠镜像治理、并发流水线哈希与轻量指纹持久化缓存、空间透视层级面包屑与闲置冷热动态色谱、菜单栏快捷小组件、网络与系统安全隐私治理、偏好碎片反查、空间透视超大冷文件原位压缩归档与外接驱动器迁移、系统深度扩展与 QuickLook/Spotlight 插件残存治理、全局快捷键呼出与极速一键清理微面板等
